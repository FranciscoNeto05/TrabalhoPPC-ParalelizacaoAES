...
#pragma omp parallel for private(temp)
  for (int i = 0; i < 4; i++) {
    temp[0] = multiplicarGalois(estado[0][i], 0x0e) ^
              multiplicarGalois(estado[1][i], 0x0b) ^
              multiplicarGalois(estado[2][i], 0x0d) ^
              multiplicarGalois(estado[3][i], 0x09);
    temp[1] = multiplicarGalois(estado[0][i], 0x09) ^
              multiplicarGalois(estado[1][i], 0x0e) ^
              multiplicarGalois(estado[2][i], 0x0b) ^
              multiplicarGalois(estado[3][i], 0x0d);
    temp[2] = multiplicarGalois(estado[0][i], 0x0d) ^
              multiplicarGalois(estado[1][i], 0x09) ^
              multiplicarGalois(estado[2][i], 0x0e) ^
              multiplicarGalois(estado[3][i], 0x0b);
    temp[3] = multiplicarGalois(estado[0][i], 0x0b) ^
              multiplicarGalois(estado[1][i], 0x0d) ^
              multiplicarGalois(estado[2][i], 0x09) ^
              multiplicarGalois(estado[3][i], 0x0e);
...
void descriptografarAES(unsigned char *entrada, unsigned char *saida,
