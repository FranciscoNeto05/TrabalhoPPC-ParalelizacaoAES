...
void MisturarColunas(unsigned char estado[4][4]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        temp[2] = estado[0][i] ^ estado[1][i] ^ multiplicarGalois(estado[2][i], 2) ^ multiplicarGalois(estado[3][i], 3);
        temp[3] = multiplicarGalois(estado[0][i], 3) ^ estado[1][i] ^ estado[2][i] ^ multiplicarGalois(estado[3][i], 2);
...
void criptografarAES(unsigned char* entrada, unsigned char* saida, unsigned char* chaveExpandida) {
